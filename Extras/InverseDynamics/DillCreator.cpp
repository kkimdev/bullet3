...
int DillCreator::recurseDill(const int level, const int parent, const idScalar d_DH_in,
    ...
    m_mass[body] = btScalar(0.1) * BT_ID_POW(size, 3);
    m_body_r_body_com[body](0) = btScalar(0.05) * size;
    ...
    m_body_I_body[body](1, 1) = size_5 * btScalar(403 / 1.2e6);
    ...
    for (int i = 1; i <= level; i++) {
        idScalar d_DH = btScalar(0.01) * size;
        if (i == level) {
            d_DH = 0.0;
        }
